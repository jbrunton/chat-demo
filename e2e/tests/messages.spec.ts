...
test("user can send commands", async ({ page, menu, roomPage }) => {
  await page.goto("/");

  await menu.open();
  await menu.createRoom();

  await roomPage.sendMessage("/help");
  await expect(
    roomPage.getMessage("Type to chat, or enter one of the following commands")
  ).toBeVisible();
});
